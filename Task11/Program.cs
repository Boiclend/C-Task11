...
class Worker 
{
    ...
    public Worker(string sur, double exp, int hag, int how)
    {
        Surname = sur;
        Expirience = exp;
        HourlyWag = hag;
        ...
        if(Expirience > 1 && Expirience < 3 ) Premy = Salary / 100 * 5;
        if(Expirience > 3 && Expirience < 5 ) Premy = Salary / 100 * 8;
        if(Expirience > 5) Premy = Salary / 100 * 15;
    }
    ...
    public void ReadToFile() {
        string Info = "Surname is " + Surname + "'\n'Expirience is " + Expirience + "'\n'HourlyWag is " 
                      ...
                      + "'\n'Salary is " + Salary + "'\n'Premy is " + Premy;
        File.WriteAllText(@"C:\\Users\РомаЛиля\Desktop\C#Task11\test.txt",Info);
    }
}
